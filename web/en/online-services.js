// Frontend functions for the online services.
// Loaded before app.js because the current views use these globals.
// -- ONLINE WIRING --
...
async function onlineFetch(path, options = {}) {
  return window.AtriaOnlineBootstrap.onlineFetch(path, options, { loadConfig, loadOnlineAccount, loadOnlineSession, lang: 'en' });
}
...
async function onlineAuthFetch(path, payload) {
  return window.AtriaOnlineBootstrap.onlineAuthFetch(path, payload, { loadConfig, lang: 'en' });
}
...
function getOnlineBootstrapDepsForRealtimeHandlers() {
  return {
    ...
    encryptedFallbackText: '[encrypted message]',
...
async function bridgeDecryptPacket(packet, sharedKeyB64, peerSigPubB64, options = {}) {
  if (peerSigPubB64 && packet.sig) {
    ...
    if (!valid) throw new Error(options.lang === 'en' ? 'Invalid packet signature' : 'Firma de paquete invalida');
  }
...
function confirmReplaceLocalDataForOnlineRestore({ localKeyCount, remoteKeyCount }) {
  return confirm(`This device already has local Atria data (${localKeyCount} keys). To load your online account, Atria will replace that local data with the validated online backup (${remoteKeyCount} keys). If you have local changes that were not uploaded, cancel and export a manual backup first. Continue?`);
}
