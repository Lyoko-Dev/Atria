// Online chat and message composer views.
// Loaded before app.js because the current routes use these globals.
...
function notifyOnlineEvent(event = {}) {
  ...
  const title = event.title || 'Online activity';
...
function renderTablon() {
  ...
  chatActiveChannel = {id:'tablon', type:'tablon', name:'System board'};
  renderInnerChat();
}
...
function renderInnerChat() {
  const tabLabel = {chat:'Chat', online:'Online chat', tablon:'Board', solicitudes:'Requests', deseos:'Wishes'};
  const _comLabel = {chat:'Chat', online:'Online chat', tablon:'Board', solicitudes:'Requests', deseos:'Wishes'};
  setCrumbs([{label:'Hub',action:()=>navigateTo('hub')},{label:'Communication · '+(_comLabel[comTab]||'Chat')}]);
...
function renderComTabsHTML() {
  ...
  const tabs = [
    ...
    {id:'chat',        label:'◭ Chat'},
    {id:'tablon',      label:'◈ Board'},
    {id:'solicitudes', label:'◱ Requests', badge: pendSolic||null},
    {id:'deseos',      label:'◈ Wishes'},
  ];
  ...
  root.innerHTML = `
    ...
    <div class="chat-sidebar">
      <div class="chat-sidebar-header">
        ...
        <button class="icon-btn" id="btn-new-channel" title="New channel" style="font-size:16px">+</button>
      </div>
      <div class="chat-channel-list">
        <div class="chat-section-label" style="font-family:'DM Mono',monospace;font-size:9px;letter-spacing:.14em;text-transform:uppercase;color:var(--text-3);padding:8px 10px 4px">System</div>
        <div class="chat-channel-item${chatActiveChannel?.type==='tablon'?' active':''}" id="chat-tablon-item">
          ...
          <div class="chat-channel-name">System board</div>
        </div>

        <div class="chat-section-label" style="font-family:'DM Mono',monospace;font-size:9px;letter-spacing:.14em;text-transform:uppercase;color:var(--text-3);padding:8px 10px 4px">Channels</div>
        ${channels.map(ch => {
          ...
          return `<div class="chat-channel-item${isActive?' active':''}" data-ch="${ch.id}" data-type="channel">
            ...
            ${!DEFAULT_CHANNELS.find(d=>d.id===ch.id)?`<button class="icon-btn chat-del-ch" data-ch="${ch.id}" style="font-size:11px;opacity:.4" title="Delete channel">✕</button>`:''}
          </div>`;
        }).join('')}

        <div class="chat-dm-section">
          <div class="chat-dm-label">Direct messages</div>
          ${alters.map(a => {
          ...
          ${alters.length===0?`<div style="font-family:'DM Mono',monospace;font-size:10px;color:var(--text-3);padding:4px 10px">No other alters</div>`:''}
        </div>
      </div>
    </div>
...
function renderOnlineChatMessagesOnly() {
  ...
  list.innerHTML = msgs.length ? msgs.map(msg => {
    ...
    const senderName = senderAlter?.name || (isSelf ? (account?.systemId || 'me') : (activeFriend.displayName || activeFriend.identifier));
    const senderColor = senderAlter?.color || (isSelf ? '#5fffb0' : 'var(--accent)');
    const timeStr = new Date(msg.ts).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'});
    return `<div class="chat-msg${isSelf?' is-self':''}">
      ...
      <div class="chat-msg-body">
        ...
        <div class="chat-msg-bubble">${escC(msg.text)}${msg.pending ? ' <span style="color:var(--text-3);font-size:10px">sending...</span>' : ''}</div>
      </div>
    </div>`;
  }).join('') : `<div class="chat-empty"><div class="chat-empty-icon">@</div><div>No messages yet</div><div style="font-size:11px;color:var(--text-3)">New messages will appear here as they arrive</div></div>`;
...
function renderOnlineChatPanel(panel) {
  ...
  panel.innerHTML = `
    <div class="chat-header">
      ...
      <div class="chat-header-info">
        <div class="chat-header-name">${activeFriend ? esc(activeFriend.displayName || activeFriend.identifier) : 'Online chat'}</div>
        <div class="chat-header-desc">${activeFriend ? `Online DM${presence?.state ? ` | presence ${esc(presence.state)}` : ''}${activeMeta?.unreadCount ? ` | ${activeMeta.unreadCount} unread` : ''}` : 'Select a friend to open their online conversation'}</div>
      </div>
      <div style="margin-left:auto;display:flex;gap:8px;align-items:center">
        <div style="font-family:'DM Mono',monospace;font-size:10px;color:${backendConfigured?'#5fffb0':'var(--text-3)'}">${backendConfigured?'ONLINE_READY':'LOCAL_CACHE'}</div>
        <button class="icon-btn" id="btn-online-chat-refresh" title="Refresh online chat" style="font-size:12px">R</button>
      </div>
    </div>
    <div class="chat-messages" id="online-chat-msg-list">
      ...
      ) : `<div class="chat-empty"><div class="chat-empty-icon">@</div><div>No friend selected</div><div style="font-size:11px;color:var(--text-3)">Add friends from Hub → Online → Friends</div></div>`}
    </div>
    <div class="chat-input-area">
      <div class="chat-input-wrap" style="display:flex;align-items:flex-end;gap:6px">
        <button id="online-chat-sender-btn" title="Change speaking alter" style="flex-shrink:0;width:32px;height:32px;border-radius:50%;border:2px solid ${onlineSender?.color || '#5fffb0'};background:${onlineSender?.bg || 'rgba(95,255,176,.12)'};font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center;overflow:hidden;padding:0">${renderOnlineAlterAvatar(onlineSenderSnapshot, 32)}</button>
        <textarea class="chat-input" id="online-chat-input" placeholder="${activeFriend ? `Write to ${esc(activeFriend.displayName || activeFriend.identifier)}...` : 'Select a friend first'}" rows="1" style="flex:1" ${activeFriend ? '' : 'disabled'}></textarea>
  ...
  const send = () => {
    ...
    sendOnlineConversationMessage(activeFriend.id, text, { senderAlter })
      .then(({ mode }) => {
        ...
        renderOnlineChatPanel(panel);
        if (mode === 'remote') showToast('Online message sent');
      })
      .catch(e => {
        removeOnlineConversationMessage(activeFriend.id, tempId);
        renderOnlineChatPanel(panel);
  ...
  panel.querySelector('#btn-online-chat-refresh')?.addEventListener('click', async () => {
    ...
    try {
      ...
      renderOnlineChatPanel(panel);
      showToast(backendConfigured ? 'Online chat refreshed' : 'Online service unavailable: showing local cache');
    } catch (e) {
      showToast('Warning: ' + e.message);
    }
  });
...
function renderOnlineChatLayout() {
  ...
  root.innerHTML = `
    <div class="chat-sidebar">
      <div class="chat-sidebar-header">
        <div class="chat-sidebar-title">Online chat</div>
        <div style="font-family:'DM Mono',monospace;font-size:10px;color:#5fffb0">${friends.length} friend${friends.length!==1?'s':''}</div>
      </div>
      <div class="chat-channel-list">
        ...
        ${friends.length ? friends.map(f => {
          ...
          return `<div class="chat-channel-item${isActive?' active':''}" data-online-friend="${f.id}">
            ...
            <div style="display:flex;flex-direction:column;min-width:0;flex:1">
              ...
              <div style="font-family:'DM Mono',monospace;font-size:9px;color:var(--text-3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${preview ? esc(preview.text).slice(0,32) : 'No messages'}</div>
            </div>
            ${meta?.unreadCount ? `<div class="mtab-badge">${meta.unreadCount}</div>` : ''}
          </div>`;
        }).join('') : `<div style="font-family:'DM Mono',monospace;font-size:10px;color:var(--text-3);padding:10px">No online friends yet</div>`}
      </div>
    </div>
    <div class="chat-main">
      <div id="chat-messages-panel" style="display:flex;flex-direction:column;flex:1;overflow:hidden"></div>
