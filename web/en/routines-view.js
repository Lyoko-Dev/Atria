...
function renderRutinas() {
  setCrumbs([{label:'Hub',action:()=>navigateTo('hub')},{label:'Routines'}]);
  ...
  const title = rutinasTab === 'hoy' ? 'Today\'s routines' : rutinasTab === 'plantillas' ? 'Templates' : 'All routines';
  const subtitle = rutinasTab === 'hoy'
    ? `${doneToday}/${dueToday.length} completed today`
    : rutinasTab === 'plantillas'
      ? `${templates.length} template${templates.length!==1?'s':''} saved`
      : rutinasTab === 'adherencia'
        ? `Last 30 days`
        : `${activeList.length} active routine${activeList.length!==1?'s':''}`;

  app.innerHTML = `
    <div class="proy-view">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">
        <div>
          <div class="fin-title">◎ Routines</div>
      ...
      <div class="mem-tabs" style="margin-top:14px">
        <div class="mem-tab${rutinasTab==='hoy'?' active':''}" data-rtab="hoy">Today</div>
        <div class="mem-tab${rutinasTab==='todas'?' active':''}" data-rtab="todas">All</div>
        <div class="mem-tab${rutinasTab==='adherencia'?' active':''}" data-rtab="adherencia">Adherence</div>
        <div class="mem-tab${rutinasTab==='plantillas'?' active':''}" data-rtab="plantillas">Templates</div>
      </div>
      <div class="task-panel" style="margin-top:14px">
        <div class="task-panel-header">
          <div>
            <div class="task-panel-title">${title}</div>
            <div class="task-panel-desc">${rutinasTab==='hoy' ? 'Checklists and habits planned for today.' : 'Reusable templates to structure daily life.'}</div>
          </div>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
          <span class="proy-card-status proy-status-activo">${dueToday.length} today</span>
          <span class="proy-card-status proy-status-pausado">${activeList.length} active</span>
          <span class="proy-card-status proy-status-completado">${doneToday} completed</span>
        </div>
      </div>
      <div style="display:flex;flex-direction:column;gap:12px;margin-top:14px">
        ${rutinasTab === 'adherencia' ? renderRutinasAdherencia(activeList, date) :
          ...
          `<div class="task-panel"><div class="task-empty"><div class="task-empty-icon">◎</div><div>There is nothing in this view yet</div><button class="btn btn-primary" style="margin-top:8px" id="btn-new-routine-empty">Create routine</button></div></div>`}
      </div>
    </div>`;
  ...
  app.querySelectorAll('[data-routine-toggle]').forEach(btn => btn.addEventListener('click', () => {
    ...
    saveRoutines(list);
    showToast(routine.active ? 'Routine activated ✓' : 'Saved as template ✓');
    renderRutinas();
  }));
...
function renderRoutineCard(r, alters, date, tab) {
  ...
  const daysText = r.frequency === 'weekly'
    ? ((Array.isArray(r.daysOfWeek) && r.daysOfWeek.length) ? ROUTINE_DAY_OPTIONS.filter(d=>r.daysOfWeek.includes(d.id)).map(d=>d.short).join(' · ') : 'Weekly')
    : 'Daily';
  ...
  return `<div class="task-panel">
    <div class="task-panel-header" style="align-items:flex-start">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end">
        <button class="btn btn-ghost btn-sm" data-routine-edit="${r.id}">✎ Edit</button>
        <button class="btn btn-ghost btn-sm" data-routine-toggle="${r.id}">${r.active ? 'Save as template' : 'Activate'}</button>
        <button class="btn btn-danger btn-sm" data-routine-delete="${r.id}">✕</button>
      </div>
    </div>
    <div class="task-panel-meta" style="flex-wrap:wrap">
      <span class="proy-card-status proy-status-${r.active ? 'activo' : 'pausado'}">${r.active ? 'Active' : 'Template'}</span>
      ...
      ${tab === 'hoy' ? `<span style="font-family:'DM Mono',monospace;font-size:10px;color:var(--text-2);margin-left:auto">${steps.length ? `${progress.doneCount}/${steps.length} steps` : (progress.done ? 'Done' : 'Pending')}</span>` : ''}
    </div>
    ${tab === 'hoy' ? `<div style="display:flex;flex-direction:column;gap:8px;margin-top:12px">
        ${steps.length ? steps.map(step => `<label style="display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid var(--border);border-radius:12px;background:var(--bg-2);cursor:pointer"><input type="checkbox" data-routine-id="${r.id}" data-routine-step="${step.id}" ${progress.entry?.completedSteps?.includes(step.id) ? 'checked' : ''}><span style="color:${progress.entry?.completedSteps?.includes(step.id) ? 'var(--green)' : 'var(--text-1)'}">${escP(step.label)}</span></label>`).join('') : `<div style="padding:10px 12px;border:1px dashed var(--border);border-radius:12px;color:var(--text-2);font-size:12px">No checklist yet. Use the button to mark it done for today.</div>`}
        <div style="display:flex;justify-content:flex-end"><button class="btn ${progress.done ? 'btn-ghost' : 'btn-primary'} btn-sm" data-routine-id="${r.id}" data-routine-done="${progress.done}">${progress.done ? 'Mark pending' : 'Mark done'}</button></div>
      </div>` : `<div style="margin-top:12px;padding:12px;border:1px solid var(--border);border-radius:12px;background:var(--bg-2)"><div style="font-size:11px;color:var(--text-3);font-family:'DM Mono',monospace;text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">Checklist</div>${steps.length ? `<div style="display:flex;flex-direction:column;gap:6px">${steps.map(step => `<div style="font-size:13px;color:var(--text-1)">• ${escP(step.label)}</div>`).join('')}</div>` : `<div style="font-size:12px;color:var(--text-2)">No steps defined yet.</div>`}</div>`}
  </div>`;
}
function openRoutineModal(item) {
  ...
  openModal(`
    ...
    <div class="form-grid">
      <div class="form-row"><div class="form-label">Title</div><input type="text" id="routine-title" placeholder="e.g. morning review" value="${escP(it.title||'')}"></div>
      <div class="form-row"><div class="form-label">Description</div><textarea id="routine-desc" placeholder="What happens in this routine..." style="min-height:90px">${escP(it.desc||'')}</textarea></div>
      <div class="form-row two-col">
        <div class="form-row"><div class="form-label">Scope</div><select id="routine-scope">${ROUTINE_SCOPE_OPTIONS.map(opt => `<option value="${opt.id}" ${it.scope===opt.id?'selected':''}>${opt.label}</option>`).join('')}</select></div>
        <div class="form-row"><div class="form-label">Frequency</div><select id="routine-frequency">${ROUTINE_FREQ_OPTIONS.map(opt => `<option value="${opt.id}" ${it.frequency===opt.id?'selected':''}>${opt.label}</option>`).join('')}</select></div>
      </div>
      <div class="form-row two-col">
        <div class="form-row"><div class="form-label">Time (optional)</div><input type="time" id="routine-time" value="${escP(it.time||'')}"></div>
        <div class="form-row" style="justify-content:flex-end"><label style="display:flex;align-items:center;gap:8px;margin-top:24px;color:var(--text-1)"><input type="checkbox" id="routine-active" ${it.active!==false ? 'checked' : ''}>Active</label></div>
      </div>
      <div class="form-row"><div class="form-label">Days of the week</div><div style="display:flex;gap:8px;flex-wrap:wrap">${ROUTINE_DAY_OPTIONS.map(day => `<label style="display:flex;align-items:center;gap:6px;padding:8px 10px;border:1px solid var(--border);border-radius:999px;background:var(--bg-2);font-size:12px"><input type="checkbox" data-rday="${day.id}" ${selectedDays.has(day.id) ? 'checked' : ''}>${day.label}</label>`).join('')}</div></div>
      <div class="form-row"><div class="form-label">Assignment</div><div style="display:flex;gap:8px;flex-wrap:wrap">${alters.map(a => `<label style="display:flex;align-items:center;gap:7px;padding:8px 10px;border:1px solid var(--border);border-radius:999px;background:var(--bg-2);font-size:12px"><input type="checkbox" data-ralter="${a.id}" ${(it.alterIds||[]).includes(a.id) ? 'checked' : ''}><span>${a.emoji} ${escP(a.name)}</span></label>`).join('')}</div></div>
      <div class="form-row"><div class="form-label">Checklist (one step per line)</div><textarea id="routine-checklist" placeholder="Drink water&#10;Take medication&#10;Review today's tasks" style="min-height:120px">${escP(checklistText)}</textarea></div>
    </div>
    ...
    (ov) => {
      const title = ov.querySelector('#routine-title').value.trim() || 'Untitled routine';
      ...
      saveRoutines(list);
      showToast(isEdit ? 'Routine updated ✓' : 'Routine created ✓');
      renderRutinas();
    });
}
...
const TASK_STATUSES = [
  {id:'pendiente',   label:'Pending',   color:'var(--text-2)'},
  {id:'en-progreso', label:'In progress', color:'var(--accent-4)'},
  {id:'completada',  label:'Completada',  color:'var(--green)'},
  {id:'bloqueada',   label:'Bloqueada',   color:'var(--red)'},
];
const PROY_STATUSES = [
  {id:'activo',      label:'Active'},
  {id:'pausado',     label:'Paused'},
  {id:'completado',  label:'Completed'},
  {id:'archivado',   label:'Archived'},
];
