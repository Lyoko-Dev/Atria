...
function renderProyectos() {
  if (activeAlter && activeAlter.permissions !== undefined && !activeAlter.permissions?.proyectos) {
    document.getElementById('app').innerHTML = `<div class="empty-state" style="margin-top:60px"><div class="empty-icon">🔒</div><div>No tienes permisos para acceder a Proyectos</div></div>`;
    setCrumbs([{label:'Hub',action:()=>navigateTo('hub')},{label:'Proyectos'}]);
    return;
  }
  setCrumbs([{label:'Hub',action:()=>navigateTo('hub')},{label:'Proyectos'}]);
  if(!activeProyId) {
  ...
  app.innerHTML = `
    <div class="proy-view">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
        <div>
          <div class="fin-title">◉ Proyectos</div>
          <div class="fin-subtitle">${proyectos.length} proyecto${proyectos.length!==1?'s':''} · ${proyectos.filter(p=>p.status==='activo').length} activo${proyectos.filter(p=>p.status==='activo').length!==1?'s':''}</div>
        </div>
        <button class="btn btn-primary" id="btn-new-proy">+ Nuevo proyecto</button>
      </div>
      <div class="mem-tabs" style="margin-bottom:4px">
        <div class="mem-tab${proyTab==='proyectos'?' active':''}" data-ptab="proyectos">◉ Proyectos</div>
        <div class="mem-tab${proyTab==='responsabilidades'?' active':''}" data-ptab="responsabilidades">◈ Responsabilidades</div>
      </div>
          ...
          ${proyectos.filter(p=>p.status==='archivado').length?`
            <details style="margin-top:4px">
              <summary style="font-family:'DM Mono',monospace;font-size:10px;color:var(--text-3);cursor:pointer;padding:6px 2px;letter-spacing:.1em;text-transform:uppercase">
                ↓ Archivados (${proyectos.filter(p=>p.status==='archivado').length})
              </summary>
              <div style="display:flex;flex-direction:column;gap:8px;margin-top:8px">
                ${proyectos.filter(p=>p.status==='archivado').map(p=>renderProyCard(p,alters)).join('')}
              </div>
            </details>`:''}
          <div class="proy-add-card" id="btn-add-proy-inline">+ Nuevo proyecto</div>
        </div>
        ...
        <div id="task-panel-wrap">
          ${active ? renderTaskPanel(active, alters) :
            `<div class="task-panel"><div class="task-empty">
              ...
              <div>Selecciona o crea un proyecto</div>
              <button class="btn btn-primary" style="margin-top:8px" id="btn-new-proy-2">Crear primer proyecto</button>
            </div></div>`}
        </div>
      </div>
    </div>`;
...
function renderRespoPanel() {
  ...
  const STATUS_LABEL = {pendiente:'Pendiente','en-progreso':'En progreso',completada:'Completada',bloqueada:'Bloqueada'};
  ...
  app.innerHTML = `
    <div class="proy-view">
      ...
      <div class="mem-tabs" style="margin-bottom:4px">
        <div class="mem-tab" data-ptab="proyectos">◉ Proyectos</div>
        <div class="mem-tab active" data-ptab="responsabilidades">◈ Responsabilidades</div>
      </div>
      <div style="display:flex;flex-direction:column;gap:20px;margin-top:8px">
          ...
          return `
            <div class="card" style="padding:14px 16px">
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px">
                ...
                <div style="margin-left:auto;font-size:11px;color:var(--text-3)">${myProys.length} proyecto${myProys.length!==1?'s':''} · ${myTareas.length} tarea${myTareas.length!==1?'s':''}</div>
              </div>
              ${myProys.length ? `
                <div style="font-size:11px;text-transform:uppercase;letter-spacing:.1em;color:var(--text-3);margin-bottom:6px;font-family:'DM Mono',monospace">Proyectos responsable</div>
              ...
              ${myTareas.length ? `
                <div style="font-size:11px;text-transform:uppercase;letter-spacing:.1em;color:var(--text-3);margin-bottom:6px;font-family:'DM Mono',monospace">Tareas asignadas</div>
        ...
        }).filter(Boolean).join('') || `<div class="empty-state" style="padding:60px 20px"><div class="empty-icon">◎</div><div>Ningún alter tiene proyectos o tareas asignados</div></div>`}
      </div>
    </div>`;
    ...
    <div class="proy-card-body">
      ...
      <div class="proy-progress-wrap">
        ...
        <div class="proy-progress-label">
          <span>${prog.done}/${prog.total} tareas</span>
          <span>${prog.pct}%</span>
        </div>
      </div>
      <div class="proy-card-meta">
        ...
        <div class="proy-card-actions" style="margin-left:auto;display:flex;gap:3px;opacity:0;transition:opacity var(--transition)">
          <button class="icon-btn btn-edit-proy" data-pid="${p.id}" title="Editar">✎</button>
          <button class="icon-btn btn-del-proy" data-pid="${p.id}" title="Eliminar">✕</button>
        </div>
      </div>
    </div>
  </div>`;
...
function renderTaskPanel(p, alters) {
  ...
  // Group by status
  const groups = [
    {id:'en-progreso', label:'En progreso', color:'var(--accent-4)', icon:'⏳'},
    {id:'pendiente',   label:'Pendiente',   color:'var(--text-2)',  icon:'○'},
    {id:'bloqueada',   label:'Bloqueada',   color:'var(--red)',     icon:'✕'},
    {id:'completada',  label:'Completada',  color:'var(--green)',   icon:'✓'},
  ];
  ...
  return `<div class="task-panel">
    <div class="task-panel-header">
      ...
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;justify-content:flex-end">
        <button class="btn btn-ghost btn-sm" id="btn-task-templates">Plantillas</button>
        <button class="btn btn-primary btn-sm" id="btn-new-task">+ Tarea</button>
      </div>
    </div>
    <div class="task-panel-meta">
      ...
      <button class="btn btn-ghost btn-sm btn-edit-proy" data-pid="${p.id}" style="padding:3px 8px">✎ Editar proyecto</button>
    </div>
        ...
        return `<div class="task-group">
          ...
          ${collapsed?'':
            `<div class="task-items">
              ${items.length===0?`<div style="padding:12px 20px;font-family:'DM Mono',monospace;font-size:11px;color:var(--text-3)">Sin tareas aquí</div>`:''}
...
function renderProjectResources(project) {
  const notes = project.notes ? `<div class="project-resource-notes">${escP(project.notes).replace(/\n/g,'<br>')}</div>` : `<span style="color:var(--text-3)">Sin notas</span>`;
  const docs = (project.documents||[]).map(d=>`<a class="project-resource-link" href="${escP(d.url)}" target="_blank" rel="noopener noreferrer">↗ ${escP(d.name||d.url)}</a>`).join('') || `<span style="color:var(--text-3)">Sin documentos</span>`;
  return `<div class="project-resources"><div><div class="project-resource-label">Notas</div>${notes}</div><div><div class="project-resource-label">Documentos</div><div class="project-resource-links">${docs}</div></div></div>`;
}
...
function renderQuickTaskCapture() {
  return `<div class="quick-task-capture" style="display:flex;flex-direction:column;gap:8px;padding:12px;border:1px solid var(--border);border-radius:12px;background:var(--bg-2);margin:12px 0">
    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
      <input id="quick-task-input" class="input" type="text" autocomplete="off" placeholder="Captura rápida: llamar a Ana #salud @Luna !mañana !alta" style="flex:1;min-width:min(220px,100%)">
      <button class="btn btn-primary btn-sm" id="btn-quick-task-add">+ Tarea</button>
    </div>
    <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
      ${[['today','Hoy'],['soon','Pronto'],['later','Luego']].map(([id,label])=>`<button class="btn btn-ghost btn-sm quick-task-bucket${quickTaskBucket===id?' active':''}" data-qbucket="${id}" style="${quickTaskBucket===id?'border-color:var(--accent);color:var(--accent)':''}">${label}</button>`).join('')}
      <span style="font-size:11px;color:var(--text-3)">Usa #tags, @alter, !hoy, !mañana, !pronto, !luego, !alta</span>
    </div>
  </div>`;
}
...
function renderTaskItem(t, alters, today) {
  ...
  return `<div class="task-item" data-tid="${t.id}" draggable="true" title="Arrastra para reordenar">
    ...
    <div class="task-item-actions">
      <button class="icon-btn btn-move-task" data-tid="${t.id}" data-dir="-1" title="Subir" aria-label="Subir tarea">↑</button>
      <button class="icon-btn btn-move-task" data-tid="${t.id}" data-dir="1" title="Bajar" aria-label="Bajar tarea">↓</button>
      <button class="icon-btn btn-edit-task" data-tid="${t.id}" title="Editar">✎</button>
      <button class="icon-btn btn-del-task"  data-tid="${t.id}" title="Eliminar">✕</button>
    </div>
  </div>`;
}
...
function wireProyActions(app, alters) {
  ...
  app.querySelectorAll('.btn-del-proy').forEach(b=>b.addEventListener('click',e=>{
    ...
    showToast('Proyecto eliminado'); renderProyView();
  }));
}
...
function wireTaskPanel(app, active, alters) {
  ...
  const addQuickTask = () => {
    ...
    if (!parsed) return showToast('Aviso: Escribe una tarea');
  ...
  // Checkbox quick toggle (pendiente ↔ completada)
  ...
  app.querySelectorAll('.btn-del-task').forEach(b=>b.addEventListener('click',e=>{
    ...
    softDelete('Tarea eliminada', ()=>{}, ()=>{ const cur=loadTareas(); cur.push(tarea); saveTareas(cur); renderProyView(); });
  }));
}
