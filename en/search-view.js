...
function openSearch() {
  ...
  overlay.innerHTML = `
    <div class="search-modal">
      <div class="search-input-wrap">
        ...
        <input id="search-q" type="text" placeholder="Search the entire system..." autocomplete="off">
      </div>
      <div class="search-results" id="search-results">
        <div class="search-empty">Type to search journal, notes, tasks, calendar, routines and more</div>
      </div>
      <div class="search-hint">
        <span class="search-hint-item"><span class="search-hint-key">↑↓</span> navigate</span>
        <span class="search-hint-item"><span class="search-hint-key">↵</span> open</span>
        <span class="search-hint-item"><span class="search-hint-key">Esc</span> close</span>
      </div>
    </div>`;
  ...
  // Ctrl+K closes it if already open
...
function runSearch(q, overlay) {
  ...
  if (!q || q.length < 2) {
    res.innerHTML = '<div class="search-empty">Type to search journal, notes, tasks, calendar, routines and more</div>';
    return;
  }
  ...
  const SEARCH_LIMIT = 3; // initial items per section; "show more" expands all
  ...
  if (diarioAll.length) sections.push({
    label:'Journal', color:'#ff8ae2', icon:'◫',
    allItems: diarioAll.map(e => ({
      icon:'◫', iconBg:'rgba(255,138,226,0.15)', iconColor:'#ff8ae2',
      title: e.title||'No title',
      meta: `${alterName(e.alterId)} · ${new Date(e.ts).toLocaleDateString('en-GB',{day:'numeric',month:'short'})}`,
  ...
  if (notasAll.length) sections.push({
    label:'Notes', color:'#c4aaff', icon:'◧',
    allItems: notasAll.map(n => ({
      icon:'◧', iconBg:'rgba(196,170,255,0.15)', iconColor:'#c4aaff',
      title: n.title||'No title',
      meta: `${alterName(n.alterId)} · ${new Date(n.ts).toLocaleDateString('en-GB',{day:'numeric',month:'short'})}`,
  ...
  if (solicAll.length) sections.push({
    label:'Requests', color:'#ff8ae2', icon:'◱',
    allItems: solicAll.map(s => ({
      icon:'◱', iconBg:'rgba(255,138,226,0.12)', iconColor:'#ff8ae2',
      title: s.title,
      meta: `from ${alterName(s.fromId)} → ${alterName(s.toId)} · ${s.status}`,
      snippet: s.desc,
  ...
  if (agendaAll.length) sections.push({
    label:'Calendar', color:'#90c4ff', icon:'◷',
  ...
  if (rutinasAll.length) sections.push({
    label:'Routines', color:'#5fffb0', icon:'↻',
  ...
  if (contactosAll.length) sections.push({
    label:'Contacts', color:'#90c4ff', icon:'◎',
  ...
  if (!sections.length) {
    res.innerHTML = `<div class="search-empty">No results for "<strong>${esc(q)}</strong>"</div>`;
    return;
  }
  ...
  const renderSection = (sec, showAll) => {
    ...
    return `<div class="search-section" data-sec="${escC(sec.label)}">
      ...
      ${remaining > 0 ? `<button class="search-show-more" data-more-sec="${escC(sec.label)}" style="width:100%;text-align:left;padding:5px 10px;font-size:11px;color:var(--accent);background:none;border:none;cursor:pointer;font-family:'DM Mono',monospace">Show ${remaining} more →</button>` : ''}
    </div>`;
  };
