function renderBiblioteca() {
  ...
  setCrumbs([{label:'Hub',action:()=>navigateTo('hub')},{label:'Library'}]);
  ...
  app.innerHTML = `
    <div class="bib-view">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
        <div>
          <div class="fin-title">◫ Library</div>
          <div class="fin-subtitle">${contactos.length} contacts · ${recursos.length} resources · ${documentos.length} documents</div>
        </div>
        ${bibTab!=='salud'?`<button class="btn btn-primary" id="btn-bib-new">+ Add</button>`:''}
      </div>

      <div class="bib-tabs">
        ${[
          {id:'contactos',  label:'◎ Contacts',  n:contactos.length},
          {id:'salud',      label:'✚ Health',     n:null},
          {id:'recursos',   label:'◧ Resources',  n:recursos.length},
          {id:'documentos', label:'◪ Documents',  n:documentos.length},
...
function renderContactosTab(cont, alters) {
  ...
  if(!items.length) {
    cont.innerHTML=`<div class="empty-state" style="padding:60px 20px"><div class="empty-icon">◎</div><div>No contacts yet</div></div>`;
    return;
  }
  cont.innerHTML=`<div class="contacto-grid">${items.map(c=>{
    ...
    return `<div class="contacto-card" data-cid="${c.id}">
      <div class="contacto-actions">
        <button class="icon-btn btn-c-edit" data-cid="${c.id}" title="Edit">✎</button>
        <button class="icon-btn btn-c-del"  data-cid="${c.id}" title="Delete">✕</button>
      </div>
  ...
  cont.querySelectorAll('.btn-c-del').forEach(b=>b.addEventListener('click',e=>{
    ...
    showToast('Contact deleted'); _refreshBib('contactos');
  }));
}
...
function renderRecursosTab(cont, alters) {
  ...
  const filterHtml = cats.length>1?`<div class="bib-filters">
    <span class="bib-filter-chip${bibRecCat==='all'?' active':''}" data-rc="all">All</span>
    ...
    return `<div class="recurso-card" data-rid="${r.id}">
      ...
      <div class="recurso-body">
        ...
        <div class="recurso-meta">
          ...
          ${r.url?`<a href="${escB(r.url)}" target="_blank" class="recurso-url-link" style="font-family:'DM Mono',monospace;font-size:10px;color:var(--accent);text-decoration:none">🔗 Open</a>`:''}
        </div>
      </div>
      <div class="recurso-actions">
        <button class="icon-btn btn-r-edit" data-rid="${r.id}" title="Edit">✎</button>
        <button class="icon-btn btn-r-del"  data-rid="${r.id}" title="Delete">✕</button>
      </div>
    </div>`;
  }).join('')}</div>` : `<div class="empty-state" style="padding:40px 20px"><div class="empty-icon">◧</div><div>No resources</div></div>`);
  ...
  cont.querySelectorAll('.btn-r-del').forEach(b=>b.addEventListener('click',e=>{
    ...
    showToast('Resource deleted'); _refreshBib('recursos');
  }));
}
...
function renderDocumentosTab(cont, alters) {
  ...
  const filterHtml=cats.length>1?`<div class="bib-filters">
    <span class="bib-filter-chip${bibDocCat==='all'?' active':''}" data-dc="all">All</span>
  ...
  cont.innerHTML = filterHtml + (items.length?`<div class="doc-list">${items.map(d=>{
    ...
    return `<div class="doc-card" data-did="${d.id}">
      ...
      <div class="doc-actions">
        <button class="icon-btn btn-d-edit" data-did="${d.id}" title="Edit">✎</button>
        <button class="icon-btn btn-d-del"  data-did="${d.id}" title="Delete">✕</button>
      </div>
    </div>`;
  }).join('')}</div>` : `<div class="empty-state" style="padding:40px 20px"><div class="empty-icon">◪</div><div>No documents</div></div>`);
  ...
  cont.querySelectorAll('.btn-d-del').forEach(b=>b.addEventListener('click',()=>{
    ...
    showToast('Document deleted'); _refreshBib('documentos');
  }));
}
