...
- Language switcher (ES / EN) now shows the active button highlight correctly on all devices.
- Mobile nav bar buttons no longer appear with unexpected native styling on some browsers.
- Onboarding: pronouns field now pre-fills correctly when editing an existing alter; step 2 scrolls to the top as expected.
- Config section cards (Personalización, Datos, Emociones, etc.) no longer disappear due to a CSS specificity conflict with the button reset rule.
- Emergency contacts in the SOS panel no longer overflow the card when a contact has multiple phone numbers or emails — buttons now wrap inside the contact row and long values are truncated with ellipsis.

## Changed

- Font size selector in Config → Personalización now applies the change instantly (zoom-based scaling) without requiring a page reload. Small = 0.88×, Medium = 1×, Large = 1.13×.
